...
function onInstanceAndConfig(defer, meanioinstance, config, done) {
  ...
  mongoose.connect(defaultConfig.db || '', defaultConfig.dbOptions || {}, function(err) {
    if (err) {
    ...
    Q.allSettled (db_promises).then(dataBasesReady.bind(null, defer, mongoose.connection));
  });
};
